...
const insertRooms = `-- name: InsertRooms :one
INSERT INTO rooms
  ( "theme" ) VALUES
  ( $1 )
RETURNING "id"
...
func (q *Queries) InsertRooms(ctx context.Context, theme string) (uuid.UUID, error) {
	row := q.db.QueryRow(ctx, insertRooms, theme)
	var id uuid.UUID
	err := row.Scan(&id)
	return id, err
}
...
const reactToMessages = `-- name: ReactToMessages :one
UPDATE messages
...
func (q *Queries) ReactToMessages(ctx context.Context, id uuid.UUID) (int64, error) {
	row := q.db.QueryRow(ctx, reactToMessages, id)
